...
from execution import QueuePriority, QueueItem
import heapq
...
class PromptServer():
    def __init__(self, loop):
        ...
        async def get_queue(request):
            queue_info = self.prompt_queue.get_queue_info()
        ...
        async def post_prompt(request):
            ...
            # Extract priority, default to NORMAL, convert to uppercase for case-insensitivity
            priority_str = json_data.get('priority', 'NORMAL').upper()
            logging.info(f"Received priority: {json_data.get('priority', 'NORMAL')}, normalized to: {priority_str}")
            if priority_str not in QueuePriority.__members__:
                logging.warning(f"Invalid priority '{priority_str}' received, defaulting to NORMAL")
                priority_str = 'NORMAL'
            priority = QueuePriority[priority_str].value
            # Handle 'front' parameter by setting priority to URGENT
            if "front" in json_data and json_data['front']:
                priority = QueuePriority.URGENT.value
            logging.info(f"Set priority value to: {priority} ({QueuePriority(priority).name})")

            # Determine number (used for ordering within same priority)
            ...
            if "prompt" in json_data:
                ...
                if valid[0]:
                    ...
                    # Create QueueItem with priority
        ...
        async def post_queue(request):
            ...
            if "delete" in json_data:
                ...
                for id_to_delete in to_delete:
                    # Update lambda to use queue_item.prompt_id
                    delete_func = lambda a: a.prompt_id == id_to_delete
