...
# At the top of execution.py, after existing imports
...
class PromptQueue:
    def __init__(self, server):
        ...
        self.task_counter = 0
        self.queue = []  # Heap queue storing (priority, number, task_counter, queue_item)
        self.currently_running = {}  # Maps task ID to QueueItem
        self.history = {}
        self.flags = {}
        
    def get_queue_info(self):
        with self.mutex:
            running = [item.get_queue_info() for item in self.currently_running.values()]
            pending = [item[3].get_queue_info() for item in self.queue]
            return {"queue_running": running, "queue_pending": pending}
        
    # In execution.py, inside the PromptQueue class
    def peek_top_priority(self):
        with self.mutex:
            if len(self.queue) > 0:
                return self.queue[0][0]  # Return the priority (first element of the tuple)
            return None  # Return None if the queue is empty
        

    def put(self, queue_item: QueueItem):
        with self.mutex:
            # Use priority and number for ordering, task_counter for uniqueness
            heapq.heappush(self.queue, (queue_item.priority, queue_item.number, self.task_counter, queue_item))
            self.task_counter += 1
        ...
        with self.not_empty:
            while len(self.queue) == 0:
                ...
                if timeout is not None and len(self.queue) == 0:
                    return None
            # Pop the item with the lowest (priority, number, task_counter)
            _, _, _, queue_item = heapq.heappop(self.queue)
            i = self.task_counter
            self.currently_running[i] = queue_item  # Store QueueItem directly
            self.task_counter += 1
            self.server.queue_updated()
            return (queue_item, i)
    ...
    def task_done(self, item_id, history_result, status: Optional['PromptQueue.ExecutionStatus']):
        with self.mutex:
            ...
            for sensitive_val in SENSITIVE_EXTRA_DATA_KEYS:
                if sensitive_val in queue_item.extra_data:
                    queue_item.extra_data.pop(sensitive_val)

            self.history[queue_item.prompt_id] = {
            ...
            self.history[queue_item.prompt_id].update(history_result)
            self.server.queue_updated()

    def get_current_queue(self):
        with self.mutex:
            out = [x for x in self.currently_running.values()]
            # Extract QueueItem from the heap tuples
            queued = [item[3] for item in copy.deepcopy(self.queue)]
            return (out, queued)

    def get_current_queue_volatile(self):
        with self.mutex:
            running = [x for x in self.currently_running.values()]
            queued = [item[3] for item in copy.copy(self.queue)]
    ...
    def delete_queue_item(self, function):
        with self.mutex:
            for x in range(len(self.queue)):
                # function operates on QueueItem, which is at index 3 of the tuple
                if function(self.queue[x][3]):
                    if len(self.queue) == 1:
                        self.wipe_queue()
                    else:
                        self.queue.pop(x)
